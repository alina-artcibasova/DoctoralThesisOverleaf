...
Recent example of COVID-19 shows that viral \textit{in silico} modelling often primarily focuses on the second goal - describing viral transmission to assist in public health decisions. Such focus makes sense due to the time sensitive nature of a global pandemic, novel under-researched pathogen, and existing limitations in data reporting.

However, other viruses, such as, for example, influenza, have been extensively studied over the last 100 years, and have lots of structure, function, and host interaction information available. Despite this dearth of knowledge, influenza models still primary use small datasets with few observed variables to fit a small model of infection, which are rarely examined under other experimental conditions. While there is a certain benefit to the use of tractable and easily understandable models, their minimalist structure doesn't allow for inference on specific molecular targets for intervention.

Meanwhile, attempts to introduce additional knowledge of underlying intracellular processes often end up with models which may describe influenza infections in a bigger variety of experimental conditions, but also rely on many unobserved quantities and variables.
...
\input{D_chapters/0_introduction/7_MathematicalModels}

\input{D_chapters/0_introduction/8_Contributions}

%\input{D_chapters/0_introduction/9_LaTeXExamples}